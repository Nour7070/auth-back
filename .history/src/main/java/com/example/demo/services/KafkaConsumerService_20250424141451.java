...
public class KafkaConsumerService {
    ...
    public void handleFormateurStatusUpdate(Map<String, Object> response) {
        try {
            ...
            String status = ((String) response.get("status")).toUpperCase(); // Garantit le uppercase
    
        System.out.print"Réception mise à jour statut pour formateur: {} -> {}", email, status);
            ...
            log.info("Statut mis à jour pour {}: {}", email, status);
            
        } catch (IllegalArgumentException e) {
            log.error("Statut invalide reçu: {}", response.get("status"), e);
        } catch (Exception e) {
            log.error("Erreur traitement mise à jour statut", e);
        }
    }
